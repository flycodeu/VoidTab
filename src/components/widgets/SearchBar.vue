...
<template>
  <div class="relative w-[90%] md:w-full md:max-w-[520px] px-0 group z-30 mb-6 transition-all">
    <div class="flex items-center apple-glass rounded-full px-2 py-2.5 transition-all border border-transparent shadow-xl hover:shadow-2xl" :class="{ 'effect-neon': store.config.theme.neonGlow }">
      <div class="relative">
        ...
        <transition name="scale">
          <div v-if="showEngineMenu" class="absolute top-16 left-0 w-48 apple-glass rounded-2xl p-2 shadow-xl flex flex-col gap-1 z-50">
