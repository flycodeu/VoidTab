...
[![Version](https://img.shields.io/badge/version-1.0.9-0ea5e9)](./CHANGELOG.md)
...
- **隐私空间**：可将网站或分组移入本地加密空间，通过设置页入口、入口暗号或快捷键访问。
...
隐私空间使用本地密码加密保存内容，可在 `设置 → 隐私 → 隐私空间` 打开，也可在搜索框输入默认入口暗号 `:void` 或使用 `Ctrl + Shift + .` 进入。进入后可以筛选当前桌面的网站和分组移入隐私空间，也可以恢复、删除内容，修改入口暗号或移除隐私空间。
